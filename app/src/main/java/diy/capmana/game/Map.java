...
public class Map implements Parcelable {
    ...
    private Point startDivo = new Point(0, 0);
    private Point startPacman = new Point(0, 0);
     ...
     * Loads map data.
     */
    public boolean load(int resource) {
        // opens file
        ...
        try {
            ...
            for (int i = 0; i < length; i++)
                horzBounds.add((float) i / 16 * 2.0f - 1.0f);
            ...
            vertBounds.clear();
            for (int i = 0; i < length; i++)
                vertBounds.add((float) (length - 1 - i) / 16 * 2.0f - 1.0f);
